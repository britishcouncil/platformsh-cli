...
class DrupalRemoteFilesMountCommand extends ExtendedCommandBase {
  ...
  protected function execute(InputInterface $input, OutputInterface $output) {
    ...
    if (!$this->isMounted($project)) {
      ...
      $command = sprintf('sshfs %s-%s@ssh.bc.platform.sh:/app/public/sites/default/files %s/shared/files -o allow_other -o workaround=all -o nonempty -o reconnect -o umask=0000', $project->id, $input->getOption('environment'), $sharedPath);
